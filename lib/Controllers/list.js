...
List.prototype.fetch = function(req, res, context) {
  ...
  console.log("req.query.children: " + req.query.children);
  // if(context.shallow)
  //   delete options.include;

    if(context.shallow && false){
      let child_raw = req.query.children;
      if(!child_raw) {
        console.log("no child raw found");
        delete options.include;
      } else {
        let children = child_raw.split("|");
        let cleaned_include = [];
        console.log("found this many children types to request in include: " + children.length);
        for(let i=0;i<options.include.length;i++) {
          let include = options.include[i];
          console.log("include: " + include);
          if(include.as && children.includes(include.as))
          {
            console.log("found matching include: " + include);
            cleaned_include.push(include);
          }
          else{
            console.log("Not a match: " + include.as);
          }
        }
  ...
  //bug: counts with includes are higher than actual number of instances returned.
  //to fix https://github.com/sequelize/sequelize/issues/4042
  if(options.include && options.include.length > 0){
    options.distinct = true;
   console.log("distinct is true");
  }
  console.log("list query options: ", options, options.include);
