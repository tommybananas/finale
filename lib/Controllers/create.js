...
Create.prototype.write = function(req, res, context) {
  ...
  if (this.include && this.include.length) {
    _.values(self.resource.associationsInfo).forEach(function(association) {
      if (context.attributes.hasOwnProperty(association.as)) {
        ...
        console.log("WRITE look per assocation, attributes after add_to_all added",attr);
        //END CHANGE
  ...
  var creation_include = [];
  if(this.include && this.include.length > 0)
  {
    creation_include = this.include.slice();
  }
  if(context.shallow || creation_include === {} || Object.keys(creation_include).length === 0) {
    console.log("IN CREATE, DENYING CREATION_INCLUDE DUE TO SHALLOW FLAG or lack of creation include data, assigning to empty array");
    creation_include = [];
  }

  console.log("about to create with these attributes, creation_include", context.attributes,creation_include);
  return this.model
