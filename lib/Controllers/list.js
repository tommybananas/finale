...
List.prototype.fetch = function(req, res, context) {
  ...
  searchParams.forEach(function(searchData) {
    ...
    if (_.has(req.query, searchParam)) {
      ...
      searchAttributes.forEach(function(attr) {
        ...
        var query = {};
        var searchString;
        if ([Sequelize.Op.like, Sequelize.Op.notLike, Sequelize.Op.substring, Sequelize.Op.iLike, Sequelize.Op.notILike].includes(searchOperator)) {
          searchString = '%' + req.query[searchParam] + '%';
        } else if (Sequelize.Op.startsWith === searchOperator) {
          searchString = req.query[searchParam] + '%';
        } else if (Sequelize.Op.endsWith === searchOperator) {
          searchString = '%' + req.query[searchParam];
        } else {
          searchString = req.query[searchParam];
        }
  ...
  getKeys(req.query).forEach(key => {
    ...
    while (path.length > 1) {
      ...
      if (path.length === 1) {        
        include.where = { [prop]: { [Sequelize.Op.like]: `%${req.query[key]}%` } };
      }
    }
  });
