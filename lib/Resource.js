...
var Resource = function(options) {
  ...
  this.include = options.include.map(function(include) {
    return include.prototype && include.prototype.toString().indexOf('SequelizeInstance:') ? { model: include } : include;
  });
...
function autoAssociate(resource) {
  if (!resource.model.associations)
    ...
    //CHANGE -- creating array of subresource names to allow
    //for easier middleware interception on association endpoints
    resource.subResourceNames = resource.subResourceNames || [];
  ...
  _.forEach(resource.model.associations, function(association) {
    ...
    //CHANGE -- adding sub resource to a collection here for better lookup of subresources
    if(subResourceName)
      resource.subResourceNames.push(subResourceName);
  });
}
